...
mysql -h$DB_HOSTNAME -u$DB_USER -p$DB_PW -e "CREATE DATABASE IF NOT EXISTS $DB_NAME;"
mysql -h$DB_HOSTNAME -u$DB_USER -p$DB_PW -e "GRANT ALL PRIVILEGES ON $DB_NAME.* TO '$DB_USER' IDENTIFIED BY '$DB_PW';"

#cd $CA_PROVIDENCE_DIR/media/collectiveaccess && mkdir -p tilepics

sweep() {
	local ca="$ca"
	sed -i "s@define(\"__CA_DB_HOST__\", 'localhost');@define(\"__CA_DB_HOST__\", \'$DB_HOSTNAME\');@g" setup.php
